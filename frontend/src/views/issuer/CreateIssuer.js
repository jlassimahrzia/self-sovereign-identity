...
function CreateIssuer() {
  ...
  const retrieveIssuerRequestsList = async () => {
    let data = await IssuerService.getIssuerRequestList();
                      ...
                      <tr key={index}>
                        ...
                        <td>
                          <Button style={{background:"#d7363c",color:"white"}} id={item.id} disabled={item.state !== 0 ? true : false}
                           onClick={()=>{createIss(item)}} >Create Identity</Button>
                          <Button onClick={() => SendFailed(item)} id={item.id + "a"} disabled={item.state !== 0 ? true : false} >Decline Request</Button>
                        </td>
                      </tr> : ""
                   })
                }</tbody>
